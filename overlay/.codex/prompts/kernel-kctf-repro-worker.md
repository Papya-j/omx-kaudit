...
- Only set `supported=false` if the bug physically cannot be triggered from userspace in the provided guest environment (for example, hardware interrupt injection or a missing host-side device model). Do not set `supported=false` merely because the trigger is complex, race-sensitive, or requires several coordinated syscalls.
...
Output contract:
- `supported`: `false` only if a required guest-external prerequisite makes userspace triggering physically impossible.
